...
read -r -p "${BOL_GRE}You username? ${END}" USERNAME
[[ -z $USERNAME ]] && USERNAME=mamutal91 || USERNAME=$USERNAME
echo -e "${MAG}$USERNAME${END}\n"
clear
read -r -p "${BOL_GRE}You hostname? ${END}" HOSTNAME
[[ -z $HOSTNAME ]] && HOSTNAME=nitro5 || HOSTNAME=$HOSTNAME
echo -e "${MAG}$HOSTNAME${END}\n"
...
if [[ ${1} == recovery ]]; then
  ...
  sgdisk -g --clear \
    ...
    $SSD
    if [[ $? -eq 0 ]]; then
      echo "${GRE}sgdisk SUCCESS${END}"
  else
      echo "${RED}sgdisk FAILURE${END}"
      exit 1
  fi
  ...
  if [[ $? -eq 0 ]]; then
    echo "${GRE}Crypt luksFormat SUCCESS${END}"
  else
    echo "${RED}Crypt luksFormat FAILURE${END}"
    exit 1
  fi
  ...
  echo -e "\n${BOL_GRE}Criptografando partição swap - $SSD2 ${END}"
  cryptsetup open --type plain --key-file /dev/urandom $SSD2 swap
  mkswap -L swap /dev/mapper/swap
  swapon -L swap

  # Format the partitions
  echo -e "\n${BOL_GRE}Formatando EFI e $SSD3 ${END}"
  ...
  mkdir -p /mnt/{root,home/snapshots}
  umount -R /mnt
  mount -t btrfs -o subvol=root,$o_btrfs LABEL=system /mnt
  mount -t btrfs -o subvol=home,$o_btrfs LABEL=system /mnt/home
  mount -t btrfs -o subvol=snapshots,$o_btrfs LABEL=system /mnt/snapshots

  echo -e "\n${BOL_GRE}Montando partição boot - $SSD1 ${END}"
  mount $SSD1 /mnt/boot
